    ...
    if (p!=ms->p_end) {  /* end of pattern? */
        switch (*p) {
            ...
            case '$': {
                ...
                s=(s==ms->src_end)?s:nullptr;  /* check end of string */
                break;
            }
            ...
            default: dflt: {  /* pattern class plus optional suffix */
                ...
                if (!singlematch(ms,s,p,ep)) {
                    ...
                    switch (*ep) {  /* handle optional suffix */
                        case '?': {  /* optional */
                            const char *res;
                            if ((res=match(ms,s+1,ep+1))!=nullptr)
                                s=res;
                            else {
...
static int gmatch_aux(lua_State *L) {
    ...
    for (src=gm->src; src<=gm->ms.src_end; src++) {
        const char *e;
        reprepstate(&gm->ms);
        if ((e=match(&gm->ms,src,gm->p))!=nullptr && e!=gm->lastmatch) {
...
static int gmatch(lua_State *L) {
    ...
    gm->src=s; gm->p=p; gm->lastmatch=nullptr;
    ...
    for (i=0; i<l; i++) {
        if (news[i]!=L_ESC)
            ...
            if (!isdigit(uchar(news[i]))) {
                ...
                luaL_tolstring(L,-1,nullptr);  /* if number, convert it to string */
...
static int str_gsub(lua_State *L) {
    ...
    while (n<max_s) {
        ...
        if ((e=match(&ms,src,p))!=nullptr && e!=lastmatch) {  /* match? */
            n++;
...
static void checkdp(char *buff,int nb) {
    if (memchr(buff,'.',nb)==nullptr) {  /* no dot? */
