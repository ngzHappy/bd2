...
#include <time.h>
#define luai_makeseed()		cast(unsigned int, time(nullptr))
#endif
...
namespace luaL{
int default_lua_error_function(lua_State*L);
}

static void stack_init(lua_State *L1,lua_State *L) {
    int i; CallInfo *ci;
    /* initialize stack array */
    L1->stack=luaM_newvector(L,(BASIC_STACK_SIZE+__lua_before_status()),TValue);
    L1->stacksize=BASIC_STACK_SIZE;
    
    L1->stack_base=L1->stack;
    L1->stack+=__lua_before_status();

    for (i=0; i<BASIC_STACK_SIZE;++i) {
        setnilvalue(L1->stack+i);  /* erase new stack */
    }

    for (i=0; i<__lua_before_status();++i) {
        setfvalue(
            (L1->stack_base+i),
            (&luaL::default_lua_error_function));/* error callback function */
    }
...
static void freestack(lua_State *L) {
    ...
    luaE_freeCI(L);
    lua_assert(L->nci==0);
    luaM_freearray(L,L->stack_base,(L->stacksize+__lua_before_status()));  /* free stack array */
}
